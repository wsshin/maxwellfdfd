function prep_distsrc(osc, grid3d, eps_face_cell, mu_edge_cell, s_factor_cell, distsrc)
...
if ~isempty(distsrc.Jh) && ~isempty(distsrc.Jv)
	warning('FDS:srcAssign', 'Jh and Jv of "distsrc" are already assigned; they will be reassigned.');
end

A = wgmode_matrix(osc.in_omega0(), ...
	eps_face_cell, mu_edge_cell, ...
	s_factor_cell, grid3d, ...
	distsrc.normal_axis, distsrc.intercept, ...
	PML.sc);
